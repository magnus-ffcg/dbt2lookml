...
# Test 1: No filtering - should return all models
    ...
    args2 = MockArgs(include_models=['example_retail_data__fact_daily_sales'])
    ...
    args3 = MockArgs(exclude_models=['example_retail_data__fact_daily_sales'])
    parser1 = DbtParser(args1, manifest, catalog)
    all_models = parser1.get_models()
    parser2 = DbtParser(args2, manifest, catalog)
    included_models = parser2.get_models()
    ...
    # Assertions
    assert len(all_models) == 1, "Should have exactly one model"
    assert len(included_models) == 1, "Should have exactly one included model"
    assert included_models[0].name == 'example_retail_data__fact_daily_sales'
    assert len(excluded_models) == 0, "Should have no models after excluding the only one"
