...
class TestRecurseJoinsComprehensive:
    ...
    def test_single_array_join_creation(self, generator, test_model):
        ...
        structure = {
            'items': {
                'column': test_model.columns['items'],
                'children': []
            }
        }
    ...
    def test_camelcase_conversion_in_join_names(self, generator, test_model):
        ...
        test_model.columns['supplier_info'] = DbtModelColumn(
            name='supplier_info', 
            data_type='ARRAY',
            original_name='SupplierInfo'
        )
        
        structure = {
            'supplier_info': {
                'column': test_model.columns['supplier_info'],
                'children': []
            }
        }
    ...
    def test_nested_array_structure_sorting(self, generator, test_model):
        ...
        test_model.columns['items.details'] = DbtModelColumn(
            name='items.details', 
            data_type='ARRAY',
            original_name='Items.Details'
        )
        test_model.columns['items.details.specs'] = DbtModelColumn(
            name='items.details.specs', 
            data_type='ARRAY',
            original_name='Items.Details.Specs'
        )
        
        structure = {
            'items.details.specs': {
                'column': test_model.columns['items.details.specs'],
                'children': []
            },
            'items': {
                'column': test_model.columns['items'],
            ...
            'items.details': {
                'column': test_model.columns['items.details'],
                'children': []
            }
        }
    ...
    def test_original_name_fallback_handling(self, generator, test_model):
        ...
        test_model.columns['simple_array'] = DbtModelColumn(
            name='simple_array', 
            data_type='ARRAY'
        )
        ...
        structure = {
            'simple_array': {
                'column': test_model.columns['simple_array'],
                'children': []
            }
        }
    ...
    def test_complex_nested_structure_with_multiple_levels(self, generator, test_model):
        ...
        test_model.columns.update({
            'items.classification': DbtModelColumn(
                ...
                data_type='STRUCT',
                original_name='Items.Classification'
            ),
            'items.classification.categories': DbtModelColumn(
                name='items.classification.categories', 
                data_type='ARRAY',
                original_name='Items.Classification.Categories'
            ),
            'supplier_data': DbtModelColumn(
                name='supplier_data', 
                data_type='ARRAY',
                original_name='SupplierData'
            )
        })
        ...
        structure = {
            ...
            'items.classification.categories': {
                'column': test_model.columns['items.classification.categories'],
                'children': []
            },
            'supplier_data': {
                'column': test_model.columns['supplier_data'],
                'children': []
            }
        }
    ...
    def test_join_sql_format_for_nested_arrays(self, generator, test_model):
        ...
        # Add nested array
        test_model.columns['items.tags'] = DbtModelColumn(
            name='items.tags', 
            data_type='ARRAY',
            original_name='Items.Tags'
        )
        ...
        structure = {
            ...
            'items.tags': {
                'column': test_model.columns['items.tags'],
                'children': []
            }
        }
    ...
    def test_edge_case_empty_original_name_parts(self, generator, test_model):
        ...
        test_model.columns['edge_case'] = DbtModelColumn(
            name='edge_case', 
            data_type='ARRAY',
            original_name=''  # Empty original name
        )
        
        structure = {
            'edge_case': {
                'column': test_model.columns['edge_case'],
                'children': []
            }
        }
    ...
    def test_deeply_nested_parent_view_detection(self, generator, test_model):
        ...
        test_model.columns.update({
            ...
            'level1.level2.level3': DbtModelColumn(name='level1.level2.level3', data_type='ARRAY', original_name='Level1.Level2.Level3'),
        })
