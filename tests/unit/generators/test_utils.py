...
def test_get_column_name_simple():
    ...
    # Test with table_format_sql=False - now always returns ${TABLE} format
...
def test_get_column_name_nested():
    ...
    # Test with table_format_sql=False - now always returns ${TABLE} format
    assert get_column_name(column, False) == '${TABLE}.parent.child'
...
def test_get_column_name_deeply_nested():
    ...
    # Test with table_format_sql=False - now always returns ${TABLE} format
    assert get_column_name(column, False) == '${TABLE}.grandparent.parent.child'
...
def test_hash_uniqueness():
    ...
    for test_input in invalid_cases:
        ...
        assert result[6:].isalnum(), (
            f"Expected hash part to be alphanumeric, got {repr(result[6:])} " f"for input {repr(test_input)}"
        )
        # Verify uniqueness
        results.add(result)
    ...
    assert len(results) == len(invalid_cases), f"Expected {len(invalid_cases)} unique hashes, got {len(results)}"
...
def test_get_column_name_with_original_name():
    ...
    # Without table format - now always returns ${TABLE} format
    assert get_column_name(column, False) == '${TABLE}.TestColumn'
...
def test_get_column_name_fallback_without_table_format():
    ...
    # Now always returns ${TABLE} format
