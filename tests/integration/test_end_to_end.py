...
class TestNestedLkmlGeneration:
    ...
    def _compare_view_objects(self, expected_view, generated_view, view_name):
        ...
        for dg_name in expected_dgs:
            ...
            if 'datatype' in exp_dg:
                assert exp_dg['datatype'] == gen_dg.get(
                    'datatype'
        ...
        if 'sql_table_name' in expected_view:
            ...
            assert expected_table == generated_table, f"{view_name}: sql_table_name mismatch {generated_table}/{expected_table}"
    ...
    def _compare_explores(self, expected_explores, generated_explores, all_views=None):
        ...
        assert len(expected_explores) == len(
            generated_explores
    ...
    def _compare_joins(self, expected_joins, generated_joins, all_views=None):
        ...
        assert (
            expected_join_names == generated_join_names
        ...
        for join_name in expected_join_names:
            ...
            if 'relationship' in exp_join:
                assert exp_join['relationship'] == gen_join.get(
                    'relationship'
            ...
            if 'required_joins' in exp_join:
                assert exp_join['required_joins'] == gen_join.get(
                    'required_joins'
            ...
            if 'required_joins' not in exp_join:
                assert (
                    'required_joins' not in gen_join
            ...
            if 'sql' in gen_join:
                assert 'UNNEST' in gen_join['sql'], f"Join {join_name}: SQL should contain UNNEST"
    ...
    def _validate_dimension_labels(self, view_name, dim_name, expected_dim, generated_dim):
        ...
        if 'group_item_label' in expected_dim:
            assert (
                'group_item_label' in generated_dim
    ...
    def test_generate_d_item_v3_complex_structs_with_explore(self):
        ...
        args = parser.parse_args(
            [
                ...
                'tests/fixtures/data',
                '--output-dir',
                'output/test_d_item_v3/',
                '--select',
                'conlaybi_item_versioned__d_item',
                '--use-table-name',
            ]
        )
    ...
    def test_generate_f_store_sales_day_selling_entity_v1_with_explore(self):
        ...
        args = parser.parse_args(
            [
                ...
                'output/tests/',
                '--select',
                'conlaybi_consumer_sales_looker__f_store_sales_day_selling_entity',
                '--use-table-name',
            ]
        )
    ...
    def test_generate_dq_item_ebo_current_with_explore(self):
        ...
        args = parser.parse_args(
            [
                ...
                'output/tests/',
                '--select',
                'conlaybi_item_dataquality__dq_ItemEBO_Current',
                '--use-table-name',
            ]
        )
