...
from dbt2lookml.enums import LookerMeasureType, LookerValueFormatName, LookerTimeFrame, LookerJoinType, LookerRelationshipType
...
class DbtMetaLookerDimension(DbtMetaLookerBase):
    ...
    value_format_name: Optional[LookerValueFormatName] = Field(
        default=None
    ) 
    timeframes: Optional[List[LookerTimeFrame]] = Field(
        default=None
    )
...
class DbtMetaLookerMeasure(DbtMetaLookerBase):
    ...
    def validate_measure_attributes(self) -> 'DbtMetaLookerMeasure':
        ...
        if any(v is not None for v in [self.approximate, self.approximate_threshold, self.sql_distinct_key]) and measure_type != LookerMeasureType.COUNT_DISTINCT:
            raise ValueError("approximate, approximate_threshold, and sql_distinct_key can only be used with count_distinct measures")
        ...
        if self.precision is not None and measure_type not in [LookerMeasureType.AVERAGE, LookerMeasureType.SUM]:
