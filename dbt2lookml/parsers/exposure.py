...
class ExposureParser:
    ...
    def get_exposures(self, exposures_tag: Optional[str] = None, model_name: Optional[str] = None) -> List[str]:
        ...
        """
        exposures = [
            exp for exp in self._manifest.exposures.values() if isinstance(exp, DbtExposure) and exp.resource_type == 'exposure'
        ]
        if exposures_tag:
